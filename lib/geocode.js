...
function googleObjToLocation (location) {
  ...
  if (!isEmpty(location.geometry)) {
    ...
    radius = locationUtil.distance.apply(this, bounds) / 2;
  } else {
...
module.exports = function (googleMapsClient) {
  ...
  var geocodePromise = Bluebird.promisify(googleMapsClient.geocode);
  var reverseGeocodePromise = Bluebird.promisify(googleMapsClient.reverseGeocode);
  ...
  return {
    ...
    reverseGeocode: function reverseGeocode (lat, lon, optionalParams) {
      ...
      return reverseGeocodePromise(query)
        .then(function (response) {
          return formatResponse(response.json.results);
        });
    }
